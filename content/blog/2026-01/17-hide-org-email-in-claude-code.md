---
title: How to Hide Org Name & Email in Claude Code
url: blog/hide-org-email-in-claude-code
tags:
  - claude-code
status: published
date: 2026-01-17T00:00:00.000Z
qblog_id: 2878d21e-2ba9-48df-a0a9-8015371c9eb8
---

When doing a screenshot/screen recording of Claude Code. It might expose your email id & org name when you start it potentially exposing confidential information which you need to post process it. 

However, if you set `export IS_DEMO=1` in the variable and then start then it'll not show those things.

![image.png](https://images.nesin.io/f_auto,q_auto/qblog/AIEngineerGuide/2026-01/awqmaackzr9d6j2vn568)

## Reference
https://x.com/shadcn/status/2012452027511513443